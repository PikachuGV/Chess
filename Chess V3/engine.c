...
int values[] = {100, 340, 330, 500, 900, INT_MAX};
int inft = INT_MAX;
...
int pawn_pst[64] = {
    ...
    40, 40, 45, 50, 50, 40, 40, 40,
    15, 20, 50, 55, 55, 45, 20, 15,
    0,  0,  30, 55, 55,  30,  0,  0,
    5, 5, -10, 0, 0, -10, 5,  5,
...
int knight_pst[64] = {
    ...
    -40,-20,  0,  0,  0,  0,-20,-40,
    -30,  0, 30, 35, 35, 30,  0,-30,
    -30,  15, 35, 35, 35, 35,  15,-30,
    -30,  0, 35, 40, 40, 35,  0,-30,
    -30,  15, 20, 35, 35, 20,  15,-30,
    -40,-20,  0,  5,  5,  0,-20,-40,
...
void get_move_list() {
    ...
    for (int i = 0; i < 256; i++) {
        ...
        legal_move_list[i].from = 0;
        legal_move_list[i].to = 0;
        legal_move_list[i].special = 0;
        legal_move_list[i].capture = 0;
    }
...
int perft(int depth, int head) {
    ...
    for (int i = 0; i < 256; i++) {
        ...
        unmake_move();
        if (head) {
            int startIdx = get_index_list(start).list[0];
            int endIdx = get_index_list(end).list[0];
            char *files = "abcdefgh";
            int fromRow = startIdx / 8; int fromCol = startIdx % 8; 
            int toRow = endIdx / 8; int toCol = endIdx % 8;
            printf("%c%i%c%i: %i\n", files[fromCol], fromRow + 1, files[toCol], toRow + 1, n);
        }
        nodes += n;
    }
    return nodes;
...
int evaluate() {
    ...
    if (no_of_pieces <= 8) {
        ...
        score += 20 * count_bits(player_controlled_squares & 0x000003C3C000000);
        score -= 20 * count_bits(opponent_controlled_squares & 0x000003C3C000000);
    }
    ...
    for (int i = 0; i < 5; i++) {
        ...
        for (int j = 0; j < 64; j++) {
            ...
            int index = list[j];
            int change = (board[index] & black_id) ?  10 * pst[index] : 10 * pst[56 + 2 * (index % 8) - index];
...
int main() {
    char fen_str[73] = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 0";
    fen_to_position(fen_str);
    time_t start = clock();
    printf("%i\n", perft(5, 1));
    time_t end = clock();
