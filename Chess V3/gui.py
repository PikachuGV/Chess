...
pieces = ['p', 'b', 'n', 'r', 'q', 'k']
colors = ["w", "b"]
bitboards = ctypes.CDLL(os.path.dirname(__file__) + "\\bitboards.dll")
...
class Window():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.promoting:
                ...
                if collision:
                    ...
                    self.board.set_up_board(self.pieces)
                    self.get_move()
                    if bitboards.get_turn() != player and ai and not start_game:
    ...
    def get_move(self):
        ...
        if move[3] == 2:
            ...
            final_square_index = index_list(move[1])[0]
            file = "abcdefgh"[final_square_index % 8]
            rank = (final_square_index // 8) + 1
            start_square_index = index_list(move[0])[0]
            ofile = "abcdefgh"[start_square_index % 8]
            orank = (start_square_index // 8) + 1
            if move[2] == 1:
                piece = ""
            else:
            ...
            if move[4]:
                ...
                capture = "-"
            notation = f"{piece}{ofile}{orank}{capture}{file}{rank}"
            if move[3] >= 4 and move[3] != 8:
                notation += "=" + "QRNB"[move[3] - 4]
        if bitboards.get_game_state() == 1:
            notation += "#"
